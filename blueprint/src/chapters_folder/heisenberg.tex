...
\begin{proposition}[Morphisme de $k$ vers $\mathcal{H}(V)$]
    \label{prop:map_k_H}
    \uses{prop:group_H}
...
\begin{proposition}[Morphisme de $\mathcal{H}(V)$ vers $V\times V^*$]
    \label{prop:map_H_VxV*}
    \uses{prop:group_H}
...
\begin{proposition}[]
    ...
    Le morphisme définit en \ref{prop:map_H_VxV*} est surjectif.
    \begin{proof}
        C'est clair.
    \end{proof}
\end{proposition}
...
\begin{proposition}[Suite exacte]
    \label{exact_seq_h}
    \uses{prop:map_k_H,prop:map_H_VxV*}
