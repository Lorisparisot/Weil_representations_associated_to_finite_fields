...
\begin{proposition}
    ...
    \uses{def:system_of_repr_center_set,def:gkH_set_iso_kH_module,def:system_of_repr_set_center_iso_G_sigma,prop:Map_kHkG_single_apply}
...
\begin{definition}
    ...
    $g\in\mathcal{S}_{G/\mathcal{Z}_G}$ the natural element of $bigoplus\limits{s\in\mathcal{S}_{G/\mathcal{Z}_G}}\mathbb{K}[H]$,
...
\begin{definition}[$\mathbb{K}[G]$ as a direct sum]
    ...
    \leanok
    We have an isomorphism $\mathbb{K}[G]\cong \bigoplus\limits_{g\in\mathcal{S}_{G/\mathcal{Z}_G}}g\mathbb{K}[H]$
    which is $\mathbb{K}[\mathcal{Z}_G]$-linear.
    \begin{proof}
        We compose the maps \ref{def:MonoidAlgebra_direct_sum_1} and \ref{gkH_set_iso_kH_module}.
    \end{proof}
\end{definition}