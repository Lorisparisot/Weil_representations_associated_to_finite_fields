...
/-- Equivalence relation generated by the quotient of `G` by its center.-/
def system_of_repr_center := QuotientGroup.con (Subgroup.center G)

/-- A function that, given `S : Set G`, returns an element of `S` if it is nonempty, and `1` otherwise.-/
noncomputable def choose (S : Set G) : G :=
   if h : S.Nonempty then Classical.choose h else 1

#check choose
/-- A system of representatives of `G` quotiented by its center. It's created as the image of
the set of the set of classes by the function `choose`.-/
def system_of_repr_center_set : Set G := by
  let u := Setoid.classes (system_of_repr_center G).toSetoid
  have h : ∀ S ∈ u , S.Nonempty := by
    intro S hS
    unfold u Setoid.classes at hS
    obtain ⟨ h1,h2 ⟩ := hS
    use h1
    rw [h2]
    simp only [Con.rel_eq_coe, Set.mem_setOf_eq]
    exact (Con.eq (system_of_repr_center G)).mp rfl
  have : ∀ (S : u), ↑S ∈ u := by
      intro S
      simp only [Subtype.coe_prop]
  have hh := @Set.image (Set G) G (choose G) u
  exact hh
...
instance system_of_repr_center_set_is_finite : Finite (system_of_repr_center_set G) := by
  ...
  exact Finite.Set.finite_image (system_of_repr_center G).classes (choose G)
...
theorem system_of_repr_center_set_disjoint (g g' : G) (hg : g ∈ (system_of_repr_center_set G)) (hg': g' ∈ system_of_repr_center_set G) :
  (g ≠ g') → {x | (system_of_repr_center G).toSetoid x g} ∩ {x | (system_of_repr_center G).toSetoid x g'} = ∅ := by
  contrapose
  push_neg
  intro h
  ...
  obtain ⟨x, h1, h2⟩ := h
  simp at h1 h2
  unfold system_of_repr_center at h1 h2
  unfold system_of_repr_center_set choose at hg hg'
  simp at hg hg'
  obtain ⟨classe_g,h_classe_g1, h_classe_g2⟩ := hg
  have : classe_g.Nonempty := by
    rw[Set.Nonempty]
    rw[Setoid.classes] at h_classe_g1
    simp at h_classe_g1
    obtain ⟨y,hy⟩ := h_classe_g1
    rw[hy]
    use y
    simp
    exact (Con.eq (system_of_repr_center G)).mp rfl
  have bla := @if_pos (classe_g.Nonempty) (decidable_empty G classe_g) this _ (Classical.choose this) 1


  sorry

theorem system_of_repr_center_set_union : Set.univ = ⋃ (g ∈ system_of_repr_center_set G), {x | (system_of_repr_center G).toSetoid x g} := by
  refine Set.ext ?_
  intro x
  constructor
  · intro hx
...
/--The set `gkH_set` is a `k` submodule of `MonoidAlgebra k G`.-/
instance gkH_set_as_k_sub (g : G) : Submodule k (MonoidAlgebra k G) := by
  refine Submodule.mk ?_ ?_
  · refine AddSubmonoid.mk ?_ ?_
    · refine AddSubsemigroup.mk ?_ ?_
      · exact gkH_set k G g
      · intro a b ha hb
        unfold gkH_set at ha hb ⊢
        obtain ⟨ha1,ha2⟩ := ha
        obtain ⟨hb1,hb2⟩ := hb
        use (ha1 + hb1)
        rw [← ha2,<-hb2]
        ...
        exact
          LeftDistribClass.left_distrib (MonoidAlgebra.single g 1)
            (Finsupp.mapDomain Subtype.val ha1) (Finsupp.mapDomain Subtype.val hb1)
    · use 0
      unfold hmul_g_kH_kG
  ...
  · intro c kG
    simp only
    intro hkG
    unfold gkH_set at hkG ⊢
    obtain ⟨hkG1,hkG2⟩ := hkG
    simp
    rw[<- hkG2]
    use (c • hkG1)
    unfold hmul_g_kH_kG
    ...
    refine Eq.symm (MonoidAlgebra.ext ?_)
    intro x
    simp
    unfold kG_kH_Module.Map_KHKG
    simp
...
noncomputable instance gkH_set_as_kH_sub (g : G) : Submodule (MonoidAlgebra k (Subgroup.center G)) (MonoidAlgebra k G) := by
  refine Submodule.mk ?_ ?_
  · refine AddSubmonoid.mk ?_ ?_
    · refine AddSubsemigroup.mk ?_ ?_
      · exact gkH_set k G g
  ...
  · intro kH kG
    ...
    intro h1
    obtain ⟨h11,h12⟩ := h1
    unfold HSMul.hSMul instHSMul SMul.smul SMulZeroClass.toSMul DistribSMul.toSMulZeroClass
    simp
    unfold DistribMulAction.toDistribSMul MulAction.toSMul DistribMulAction.toMulAction Module.toDistribMulAction
    simp
    unfold LieAlgebra.toModule LieAlgebra.ofAssociativeAlgebra Algebra.toModule Algebra.toSMul kG_kH_Module.KG_is_KcenterG_Algebra
    simp
    unfold kG_kH_Module.SMulKHKG kG_kH_Module.Map_KHKG gkH_set
    simp
    use (kH * h11)
    have : g * (kH * h11) = kH • (g * h11) := by

      sorry
    rw[this,h12]
    unfold HSMul.hSMul instHSMul SMul.smul kG_kH_Module.SMulKHKG kG_kH_Module.Map_KHKG
    simp

/--Given a representative system `S` of `G` quotiented by its center, we have an isomorphism beetween
`MonoidAlgebra k G` and the direct sum of `g • (MonoidAlgebra k (Subgroup.center G))` for `g ∈ S`.-/
noncomputable def MonoidAlgebra_direct_sum : MonoidAlgebra k G ≃ₗ[MonoidAlgebra k (Subgroup.center G)] DirectSum (system_of_rep_center_set G) (fun g => gkH_set_as_kH_sub k G g) := by
  refine LinearEquiv.mk ?_ ?_ ?_ ?_
  · refine LinearMap.mk ?_ ?_
    · refine AddHom.mk ?_ ?_
      · sorry
...
@Ind_conj_class_fun G k _ _ _ (Subgroup.center G) (@character_as_conj_class_fun k (Subgroup.center G) _ _ (FDRep.of θ)).1  := by
  ...
  obtain ⟨S, ⟨ϕ, h⟩⟩ := h2
  have := finsuppLEquivDirectSum (MonoidAlgebra k G)
